...
function normalizeModel(name) {
  ...
  // Strip known provider prefix paths (with slashes, global for duplicates)
  ...
  // Join remaining path segments only if they came from slashes
...
function shortenModel(name) {
  ...
  if (clean.length > 22) {
    ...
    if (parts.length > 1) {
      // If there's a path, show just the model part
...
async function loadData() {
  ...
  try {
    ...
    // Fetch records for model breakdown and multi-colored chart
  ...
  app.innerHTML = `
    <header class="profile-header">
      <div class="profile-info">
        <img src="https://avatars.githubusercontent.com/u/173241093?v=4" alt="avatar" class="avatar">
        <h1 class="username">nico2525nn</h1>
      </div>
      <button class="share-btn" onclick="navigator.clipboard?.writeText(location.href)">
        <svg viewBox="0 0 256 256"><path d="M165.66,90.34a8,8,0,0,1,0,11.32l-64,64a8,8,0,0,1-11.32-11.32l64-64A8,8,0,0,1,165.66,90.34ZM215.6,40.4a56,56,0,0,0-79.2,0L106.34,70.45a8,8,0,0,0,11.32,11.32l30.06-30a40,40,0,0,1,56.57,56.56l-30.07,30.06a8,8,0,0,0,11.31,11.32L215.6,119.6a56,56,0,0,0,0-79.2ZM138.34,174.22l-30.06,30.06a40,40,0,1,1-56.56-56.57l30.05-30.05a8,8,0,0,0-11.32-11.32L40.4,136.4a56,56,0,0,0,79.2,79.2l30.06-30.07a8,8,0,0,0-11.32-11.31Z"/></svg>
        Share
      </button>
    </header>
    ...
    <div class="footer" style="border-top:1px solid var(--border)">
      ${stats.firstDate} &ndash; ${stats.lastDate} &middot; ${stats.daily?.length || 0} active days
    </div>
  `;
}
...
function renderModelBreakdown(records) {
  ...
  sorted.slice(0, 10).forEach(([model, tokens], i) => {
    const pct = (tokens / total * 100).toFixed(1);
    const input = modelInput[model] || 0;
    const output = modelOutput[model] || 0;
    html += `
      <div class="breakdown-item">
        ...
        <span class="model-tokens">${formatTokens(tokens)} (${pct}%) &middot; in ${formatTokens(input)} / out ${formatTokens(output)}</span>
      </div>`;
  });
  html += '</div>';
...
function drawMultiChart(daily, records) {
  ...
  // Build stacked data: aggregate by normalized model from the daily byModel
  // stats.daily.byModel has models without prefixes already
  ...
  for (const model of visibleModels) {
    ...
    // Bottom of the stack for this model
    let baseLine = [];
    for (let i = 0; i < dataPoints.length; i++) {
      let below = 0;
...
// Model visibility toggle for chart
...
// Legacy single-color chart as fallback
