...
function drawMultiChart(daily, records) {
  ...
  // Smooth line helper - Catmull-Rom to cubic bezier
  function smoothCurve(ctx, pts, close) {
  ...
  for (const model of visibleModels) {
    ...
    for (let i = 0; i < dataPoints.length; i++) {
      const tx = xPos(i);
      const ty = pad.top + plotH - ((baseLine[i] + (dataPoints[i].models[model] || 0)) / maxVal) * plotH;
      topPts.push({ x: tx, y: ty });
      const by = pad.top + plotH - (baseLine[i] / maxVal) * plotH;
      botPts.push({ x: tx, y: by });
    }

    // Fill area: smooth top, straight bottom
    ctx.beginPath();
    if (topPts.length >= 2) {
      smoothCurve(ctx, topPts);
      ctx.lineTo(botPts[botPts.length - 1].x, botPts[botPts.length - 1].y);
      // Bottom: reverse straight
      ...
      // Stroke top edge with smooth curve
      ctx.beginPath();
      smoothCurve(ctx, topPts);
